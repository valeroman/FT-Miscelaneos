...
class MapNotifier extends StateNotifier<MapState> {
  ...
  Stream<(double, double)> trackUser() async* {
    await for( final pos in Geolocator.getPositionStream() ) {
      yield( pos.latitude, pos.longitude ); //pos.speed
    }
  }
  ...
  goToLocation( double latitude, double longitude ) {
    ...
    final newPosicion = CameraPosition(
      target: LatLng(latitude, longitude),
      zoom: 15
    );

    state.controller?.animateCamera( CameraUpdate.newCameraPosition( newPosicion ) );
  }
  ...
  void addMarker( double latitude, double longitude, String name ) {
    final newMarker = Marker(
      ...
      infoWindow: InfoWindow(
        title: name,
        snippet: 'Esto es un snippet del info window'
      )
    );
